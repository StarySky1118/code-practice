...
{// 自下而上、从右到左层次遍历二叉树T
 /*
	基本思想：使用一个辅助栈和一个辅助队列。首先将根结点入队，出队将出队结点压栈，同时将出队结点左右孩子入队。
	将最终得到的栈依次弹栈即可。
	*/
	linkStack S; // 辅助栈
	linkQueue Q; // 辅助队列
...
{// 返回二叉树T的高度
 /*
	基本思想：使用中序遍历方法，二叉树T的高度即为栈中元素最大个数。
	*/
	linkStack S; // 辅助栈
	BiNode *p=T; // 遍历指针
	BiNode *pre=NULL; // 指向上一个输出的结点
	int height=0; // 二叉树实时高度
	int maxHeight=0; // 二叉树最大高度
	AElem x; // 工作指针
	...
	while(p!=NULL||(!(StackIsEmpty(S)))) // p非空或栈非空时
	{
		if(p!=NULL) // 遇到新结点
		{
			Push(S, p); // 压栈
			height++; // 二叉树实时高度加1
			if(height>maxHeight) // 二叉树当前高度大于最大高度
				maxHeight = height; // 修改二叉树最大高度
			p = p->lchild; // 向左下进发
		}// if
		else // p为空
		{
			GetTop(S, &x); // 获取栈顶元素
			if(x->lchild==NULL||x->rchild==pre) // 栈顶结点右孩子无需遍历或者已遍历
			{
				Pop(S, &x); // 弹栈
				height--; // 二叉树实时高度减1
				pre = x; // 修改pre
