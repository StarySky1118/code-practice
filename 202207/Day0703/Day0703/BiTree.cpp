...
{// 将二叉树T所有结点左右子树交换
 /*
	基本思想：使用递归方式，首先对左子树递归调用，然后对右子树递归调用，最后交换根结点左右孩子。
 */
	BiNode* temp=NULL; // 工作变量
	if (T != NULL) // T非空时
	{
		Exchange(T->lchild); // 对左子树递归调用
		Exchange(T->rchild); // 对右孩子递归调用
		temp = T->lchild;
		T->lchild = T->rchild;
		T->rchild = temp;
	}// if
...
{// 返回二叉树T先序遍历序列中序号为k的结点数据域
 /*
	基本思想：使用辅助栈，使用指针p遍历二叉树所有结点（默认指向根结点）。在p非空或栈非空时，进行如下循环：
	若p非空，将p指针入栈，结点计数加1，若结点计数与序号相等，记录数据，退出循环；若p为空，弹栈并使p前往
	弹栈元素右孩子。
 */
	linkStack S; // 辅助栈
	BiNode* p = T; // 遍历指针
...
{// 输出指针s所指结点所有祖先
 /*
	基本思想：按后序遍历整个二叉树，如遇到指针s，停止遍历。依次输出栈中所有元素。
 */
	linkStack S; // 辅助栈
	BiNode* p = T; // 遍历指针
	AElem x = NULL; // 工作指针
	BiNode* pre = NULL; // 上一个输出的结点指针
	...
	while (p != NULL || !StackIsEmpty(S)) // p非空或栈非空时
	{
		...
		{
			GetTop(S, &x); // 获取栈顶元素
			if (x->rchild == NULL || x->rchild == pre) // 栈顶元素右孩子无需遍历或者遍历过
			{
				Pop(S, &x); // 弹栈
				pre = x; // 修改pre
			}// if
